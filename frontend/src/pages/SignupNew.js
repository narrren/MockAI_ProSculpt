...
const SignupNew = ({ onSignupSuccess, onSwitchToLogin }) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 'otp_sent' || response.data.status === 'success') {
        setStep('otp');
      ...
      setError(err.response?.data?.message || 'Failed to register. Please check if the backend server is running.');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        if (token) {
          ...
          console.log('[SignupNew] Token stored:', token.substring(0, 20) + '...');
        } else {
          console.error('[SignupNew] No token in response!', response.data);
        }
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="auth-content">
          ...
          <form className="auth-form" onSubmit={step === 'email' ? handleRegister : handleVerifyOtp}>
            {step === 'email' ? (
              <>
            ...
            <div className="auth-actions">
              <button 
                ...
                disabled={loading}
              >
                {loading ? 'Please wait...' : (step === 'email' ? 'Create Account' : 'Verify Code')}
              </button>
